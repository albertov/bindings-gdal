  ...
  describe "RasterBand IO" $ do
    ...
    describe "blockSource and blockSink" $ do
      ...
      it "can zip two bands" $ do
        ...
        flushCache ds
        let source3 = getZipSource $
              fun <$> ZipSource source1 <*> ZipSource source2
            source1 = blocks =$= blockConduit b1
            source2 = blocks =$= blockConduit b2
            blocks = allBlocks b3
            fun = U.zipWith (+)
        runConduit (zipSources blocks source3 =$= blockSink b3)
