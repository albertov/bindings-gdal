...
import Data.Bit (Bit, fromBool, toBool)
...
newtype instance U.Vector    (Value a) =  V_Value (U.Vector Bit, U.Vector a)
newtype instance U.MVector s (Value a) = MV_Value (U.MVector s Bit, U.MVector s a)
instance U.Unbox a => U.Unbox (Value a)


instance M.MVector U.MVector a => M.MVector U.MVector (Value a) where
  ...
  basicUnsafeWrite (MV_Value (x,v)) i a =
    case a of
      NoData ->
        M.basicUnsafeWrite x i (fromBool False)
      Value a' -> do
        M.basicUnsafeWrite x i (fromBool True)
...
instance G.Vector U.Vector a => G.Vector U.Vector (Value a) where
